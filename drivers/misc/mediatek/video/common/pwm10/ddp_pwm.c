...
static unsigned int pwm_max_brightness = 0;
#endif
...
int disp_pwm_get_cust_led(unsigned int *clocksource, unsigned int *clockdiv)
{
	...
	if (!led_node) {
		...
		ret = of_property_read_u32(led_node, "lge.pwm_max_brightness",&max_brightness);

		if (!ret)
			pwm_max_brightness = max_brightness;
		else
			PWM_ERR("led dts can not get pwm max brightness data, set default data = %d\n",pwm_max_brightness);
	}
	...
	enum disp_pwm_id_t id, int enabled)
{
	...
	if (enabled) {
		...
		chargepump_set_backlight_level(pwm_max_brightness);
#endif
