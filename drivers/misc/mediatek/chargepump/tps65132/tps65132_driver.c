...
static unsigned char dsv_data[3] = {0x14,0x14,0x03};
 ...
 * Extern Area
 *****************************************************************************/
static int tps65132_write_byte(unsigned char addr, unsigned char *value, u8 len)
{
    int ret = 0;
    ...
    ret = i2c_smbus_write_block_data(tps65132_i2c_client, addr, len, value);
...
{
    tps65132_write_byte(TPS65132_VPOS_ADDR, &dsv_data[POSCTRL], 1);
    tps65132_write_byte(TPS65132_VNEG_ADDR, &dsv_data[NEGCTRL], 1);
    tps65132_write_byte(TPS65132_APPS_ADDR, &dsv_data[CONTROL], 1);
...
static ssize_t sysfs_show_dsv(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    n +=  sprintf(buf+n, "VPOS = 0x%x\n",data);
    err = tps65132_read_byte(TPS65132_VNEG_ADDR, &data);
    n +=  sprintf(buf+n, "VNEG = 0x%x\n",data);
