...
static int __init lcm_name_setup(char *value)
{
    ...
    if(value != NULL) {
	    if (strlen(value) < sizeof(lcm_name)) {
		    ...
		    printk("[Touch] buffer oveflow of value(lcm_name) : strlen(value) = %d/ sizeof(value) = %d\n",
...
static int __init device_name_setup(char *value)
{
	...
	if (value != NULL) {
		...
		printk("[Touch] device_name: %s\n", device_name);
		    ...
		    printk("[Touch] buffer oveflow of value(device_name) : strlen(value)= %d/ sizeof(value) = %d\n",
				    strlen(value), sizeof(value));
	    }
	} else {
...
__setup("lge.wm_id=", lge_set_wm_id);
