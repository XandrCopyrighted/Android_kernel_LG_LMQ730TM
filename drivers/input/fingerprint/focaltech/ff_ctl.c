...
static int ff_probe(struct spi_device *spi)
{
	...
	if (IS_ERR(vdd))
	{
		FF_LOGE("Didn't find vcc_core regulator\n");
		vdd = NULL;
	}

	if (vdd != NULL)
	{
		err = regulator_set_voltage(vdd, 2800000, 2800000);

		if (err < 0)
		{
			FF_LOGE("Didn't set voltage err = %d\n", err);
			regulator_put(vdd);
			return -1;
		}

		FF_LOGI("regulator_set_voltage: vdd err = %d\n", err);

		err = regulator_enable(vdd);

		if (err < 0)
		{
			FF_LOGE("Didn't enable voltage err = %d\n", err);
			regulator_put(vdd);
			return -1;
		}

		FF_LOGI("regulator_enable: vdd err = %d\n", err);
	}
