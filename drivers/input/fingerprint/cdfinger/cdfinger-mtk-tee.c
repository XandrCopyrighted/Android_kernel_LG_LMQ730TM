...
static void cdfinger_power_on(struct cdfinger_data *cdfinger)
{
	...
	if (ret < 0) {
		CDFINGER_ERR("cdfinger: Didn't enable voltage\n");
		return ; //
	}
...
static int cdfinger_probe(struct spi_device *spi)
{
	...
	cdfinger->vdd = regulator_get(NULL, "irtx_ldo");

	if (IS_ERR(cdfinger->vdd))
	{
		CDFINGER_ERR("cdfinger: Didn't find vcc_core regulator\n");
		cdfinger->vdd = NULL;
		//goto free_regulator;
	}

	if (cdfinger->vdd != NULL)
	{
		status = regulator_set_voltage(cdfinger->vdd, 2800000, 2800000);

		if (status < 0)
		{
			CDFINGER_ERR("cdfinger: Didn't set voltage\n");
			goto free_regulator;
		}

		CDFINGER_DBG("cdfinger regulator_set_voltage: vdd status = %d\n", status);
	}
