...
static bool get_use_ext_ldo(struct platform_device *pdev)
{
	...
	return of_property_read_bool(np, "use_ext_ldo");
}
...
static int egistec_probe(struct platform_device *pdev)
{
	...
	use_ext_ldo = get_use_ext_ldo(pdev);

	dev_info(dev, "%s: use_ext_ldo = %d\n", __func__, use_ext_ldo);

	if (!use_ext_ldo) {
		struct regulator *vdd = NULL;
		int err = 0;
		vdd = regulator_get(NULL, "irtx_ldo");

		if (IS_ERR(vdd)) {
			dev_err(dev, "%s: Didn't find vcc_core regulator\n", __func__);
			vdd = NULL;
		}

		if (vdd != NULL) {
			err = regulator_set_voltage(vdd, 3200000, 3200000);

			if (err < 0) {
				dev_err(dev, "%s: Didn't set voltage err = %d\n", __func__, err);
				regulator_put(vdd);
				goto egistec_probe_failed;
			}

			dev_info(dev, "%s: regulator_set_voltage: vdd err = %d\n", __func__, err);

			err = regulator_enable(vdd);

			if (err < 0) {
				dev_err(dev, "%s: Didn't enable voltage err = %d\n", __func__, err);
				regulator_put(vdd);
				goto egistec_probe_failed;
			}

			dev_info(dev, "%s: regulator_enable: vdd err = %d\n", __func__, err);
		}
	}
