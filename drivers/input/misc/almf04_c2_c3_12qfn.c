...
static int Backup_CalData(struct i2c_client *client)
{
	...
	{
		PERR("almf04: Invalid cal data, Not back up this value.");
		return -1;
	}
...
static unsigned char load_firmware(struct almf04_data *data, struct i2c_client *client, const char *name)
{
	...
	if(restore)
	{
		...
		if(ret) PERR("restore fail ret: %d",ret);
	}
		...
		const char *buf, size_t count)
{
	...
	PINFO("almf04_store_checkallnear %d\n",check_allnear);
	return count;
}
		...
		unsigned long delay)
{
	...
	if (gpio_get_value(data->platform_data->chip_enable) == 0) { // if power on
		...
		if (atomic_read(&pm_suspend_flag) == DEV_PM_RESUME) {
			...
			if (ret < 0) {
				PERR("ALMF04 queue_work fail, ret = %d", ret);
			}
		} else {
...
static void almf04_work_handler(struct work_struct *work)
{
    ...
    else
    {
        if (gpio_get_value(data->platform_data->chip_enable) == 1) { // if power off
			PINFO("ALMF04 sensor is already power off : touch_out(0x%x)", data->touch_out);
			return;
	    }
		...
		if(get_bit(data->touch_out, 7) == 0)
		{
			...
			if (irq_state == 0 && (data->touch_out == (CH3_FAR | CH2_FAR | CH1_FAR)))
			{
			   ...
			   PINFO("ALMF04 [%d]FAR ", gpio_get_value(data->platform_data->irq_gpio));
			}
			/* NEAR */
			else
			{
			   ...
			   PINFO("ALMF04 [%d]NEAR ", gpio_get_value(data->platform_data->irq_gpio));
			}
		}
		//==============================================================================================//
		/* Scenario Mode Touch */
		...
		else
		{
			...
			if (irq_state == 0 && ((data->touch_out & 0x43) == SNR_FAR))		//Default Near Bit6 --> 1
			{
			   ...
			   PINFO("ALMF04 [%d]FAR(SM) ", gpio_get_value(data->platform_data->irq_gpio));
			}
			/* NEAR */
			else
			{
			   ...
			   PINFO("ALMF04 [%d]NEAR(SM) ", gpio_get_value(data->platform_data->irq_gpio));
			}
		}
		//==============================================================================================//
#else
	...
	#endif
        PINFO("ALMF04 Work Handler done");
...
static int sensor_regulator_configure(struct almf04_data *data, bool on)
{
	...
	if (IS_ERR(pdata->vcc_ana)) {
		...
		PERR("Regulator get failed vcc_ana rc=%d\n", rc);
		return rc;
	}
	...
	if (regulator_count_voltages(pdata->vcc_ana) > 0) {
		...
		if (rc) {
			PERR("regulator set_vtg failed rc=%d\n", rc);
...
static int sensor_platform_hw_init(struct i2c_client *client)
{
	...
	if (gpio_is_valid(data->platform_data->irq_gpio)) {
		...
		PERR(" ALMF04 irq gpio not provided\n");
	}
		...
		const struct i2c_device_id *id)
{
	...
	if (!data->input_dev_cap) {
		err = -ENOMEM;
		PERR("ALMF04 Failed to allocate input device!\n");
